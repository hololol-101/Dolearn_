/**
 * script_base.js
 * JavaScript jQuery 공통 스크립트
 * 20210121 | @m | 최초 등록
 * 20210121 | @m | 요구반영. 결함개선. 고도화.
 * ~20210204 | @m |
 */

/*! JavaScript jQuery v0.999 | (c) 20210121~. 20210121. 20210204 | by @m */
...
/** ◇◆ 토글심플. v0.2 | 20210114. 20210119. MoonYoungShin.
 * 미래 생성 요소에도 적용
 */
(MJS.doSimpleToggle = function(selector){
	var my = selector,
		b = '.toggle-b', // 토글버튼
		c = '.toggle-c', // 토글대상
		close = '.toggle-close'; // 닫기버튼
	// 클릭
	$(document).on('click', my+' '+b, function(e){
		e.preventDefault();
	...
	$(document).on('click', my+' '+close, function(e){
		e.preventDefault();
...
/** ◇◆ 토글. v0.8.7.1 | 20150520~ 20201019. 20210111. MoonYoungShin.
...
//(function doToggle(selector){
	$('.toggle').not('.donot').each(function(){ // 20150515. 토글 동작 안하기 추가
		...
		// 20190129. 20210111
		var duration = ($my.data('duration'))? $my.data('duration') : 200; // ((400
