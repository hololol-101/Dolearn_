...
class DB_mysql_class {
    ...
    */
        ...
        $this->dbHost = 'localhost';        // 호스트
        $this->dbUser = 'dolearn';        // DB 계정
        $this->dbPass = 'dolearn!1';        // DB 패스워드
        $this->dbName = 'dolearn';        // DB 네임

        $this->conn = new mysqli($this->dbHost, $this->dbUser, $this->dbPass) OR die('데이터베이스 연결에 실패하였습니다.');
...
//======================== 별도 하나씩 실행하는 스크립터용 끝
     ...
     *******************************************/
    function getPageIndex($recordPerPage,$pagePerBlock,$currentPage,$imgprev,$imgnext)
    {
        ...
        ##PREVIOUS BLOCK
            ...
            $pageIndex['htmlCode'] = '<nav class="showing_nav"><ul class="pagination">
            <li>
            <a href="'.$_SERVER['PHP_SELF'].'?'.$QUERY_STRING.'&page=1" data-load="" class="waves-effect" title="맨처음">
        ...
        {
            $pageIndex['htmlCode'].= '<li>
            <a href="'.$_SERVER['PHP_SELF'].'?'.$QUERY_STRING.'&page='.( ($pageIndex['currentBlock']-2)*$pageIndex['pagePerBlock']+1).'" data-load="" class="waves-effect" title="Prev Page">
        ...
        for($i=$pageIndex['firstPage']; $i<=$pageIndex['lastPage']; $i++)
        {
          ...
          if ( $i == $pageIndex['firstPage'] ) $class_temp = ' class="first"';
            ...
            if($i<=$pageIndex['totalPage'])
            {
                ...
                {
                    $pageIndex['htmlCode'].='<li class="active"><a title="현재 '.$i.' 페이지"  class="waves-effect">'.$i.'</a></li> ';
                }else{
                    $pageIndex['htmlCode'].='<li class=""><a href="'.$_SERVER['PHP_SELF'].'?'.$QUERY_STRING.'&page='.$i.'" data-load="" class="waves-effect hidden-xs">'.$i.'</a></li>';
                }
            }
        }
        ...
        {
            $pageIndex['htmlCode'].='<li>
            <a href="'.$_SERVER['PHP_SELF'].'?'.$QUERY_STRING.'&page='.($pageIndex['currentBlock']*$pageIndex['pagePerBlock']+1).'" data-load="" class="waves-effect" title="Next Page">
        ...
        $pageIndex['htmlCode'] .= '<li>
        <a href="'.$_SERVER['PHP_SELF'].'?'.$QUERY_STRING.'&page='.$pageIndex['totalPage'].'" data-load="" class="waves-effect" title="맨마지막">
