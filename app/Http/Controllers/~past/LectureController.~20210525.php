...
class LectureController extends Controller{
    public function lectureList(Request $request) {
        ...
        $lectureList = DB::select('SELECT idx, title, save_thumbnail, rating, student_cnt, tags, owner_name, free_yn FROM lecture'.$where.$where2.$where3.$orderBy.' limit 8');
        ...
        return view('sub.lecture.lecture_list', compact('lectureList', 'lectureTagArr', 'bcateList', 'scateList', 'bcateName', 'scateName'));
    }
    ...
    public function getLectureData(Request $request) {
        ...
        $lastLectureIdx = $request->post('last_lecture_idx', '');
        ...
        if ($lastLectureIdx != '') {
            if ($where == '') {
                $where2 = ' WHERE idx < '.$lastLectureIdx;
            } else {
                $where2 = ' AND idx < '.$lastLectureIdx;
            }
        }
        ...
        if ($keyword != '') {
            if ($where == '' && $where2 == '' && $where3 == '') {
                $where4 = ' WHERE subject LIKE "%'.$keyword.'%"';
            } else {
                $where4 = ' AND subject LIKE "%'.$keyword.'%"';
            }
        }

        $query = 'SELECT idx, title, save_thumbnail, rating, student_cnt, tags, owner_name, free_yn FROM lecture'.$where.$where2.$where3.$where4.$orderBy.' limit 8';
    ...
    public function lectureDetail(Request $request) {
        ...
        $operationLectureList = DB::select('SELECT idx, title, save_thumbnail, student_cnt FROM lecture WHERE owner_id = "'.$lectureDetail->owner_id.'" AND idx != "'.$idx.'"');

        $bchapterList = DB::select('SELECT * FROM b_chapter WHERE lecture_idx='.$idx.'');
        $schapterList = DB::select('SELECT * FROM s_chapter WHERE lecture_idx='.$idx.'');
        $curriSchapterList = array();
        $curriVideoList = array();

        foreach ($bchapterList as $bchapter) {
            $schapList = DB::select('SELECT * FROM s_chapter WHERE lecture_idx='.$idx.' AND bchap_id="'.$bchapter->bchap_id.'"');
        ...
        // echo '<pre>'.json_encode($curriVideoList, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE).'</pre>';

        return view('sub.lecture.lecture_detail', compact('lectureDetail', 'bchapterList', 'curriSchapterList', 'curriVideoList', 'countVideo', 'operationLectureList'));
    }
    ...
    public function courseApplication(Request $request) {
        ...
        try {
            ...
            // 기존 생성된 커리큘럼 정보 조회
            $curriculumList = DB::table('curriculum')->where('lecture_idx', $idx)->get();
    ...
    public function addNewLecture(Request $request) {
        ...
        // 커리큘럼
            ...
            if (count($tempCurriculumResult) > 0) {
                $bchapterList = DB::table('tempsave_b_chapter')->where('session_id', $sessionId)->where('user_id', $userId)->get();
                $schapterList = DB::table('tempsave_s_chapter')->where('session_id', $sessionId)->where('user_id', $userId)->get();
                $videoInfoList = DB::select('SELECT curri.bchap_id, curri.schap_id, curri.preview_yn, video.analysis_yn, video.uid, video.title FROM tempsave_curriculum curri, tempsave_video video WHERE curri.video_idx = video.idx AND curri.session_id = "'.$sessionId.'" AND curri.user_id = "'.$userId.'"');
            }
    ...
    function addNewLectureFinal() {
        ...
        try {
            ...
            // 임시 저장된 대단원 데이터 조회
            $tempBchapList = DB::table('tempsave_b_chapter')->where('session_id', $sessionId)->where('user_id', $userId)->get();

            // 임시 저장된 대단원 데이터 조회
            $tempSchapList = DB::table('tempsave_s_chapter')->where('session_id', $sessionId)->where('user_id', $userId)->get();
            ...
            foreach($tempCurriculumList as $key => $tempCurriculum) {
                $videoIdx = DB::table('_youtube_fulldata_temp')->select('uid')->where('uid', $tempCurriculum->video_id)->get();
