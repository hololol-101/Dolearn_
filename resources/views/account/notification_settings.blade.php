...
<div class="cp1notice1set1">
	...
	<li class="li1">
		...
		<div class="w2">
			<a href="javescript:void(0);" class="cp1switch1 switch toggle fw4 " name="notice#1">
	...
	<li class="li1">
		...
		<div class="w2">
			<a href="javescript:void(0);" class="cp1switch1 switch toggle fw4 "name="notice#2">
				<span class="t1 sw-off">OFF</span>
				<span class="t1 sw-on">ON</span>
				<i class="ic1"></i>
			</a>
    ...
    <li class="li1">
		...
		<div class="w2">
			<a href="javescript:void(0);" class="cp1switch1 switch toggle fw4 " name="notice#3">
				<span class="t1 sw-off">OFF</span>
				<span class="t1 sw-on">ON</span>
				<i class="ic1"></i>
			</a>
    ...
    <!-- TODO:로그인 유저 정보에서 role이 유튜버(youtuber)인 경우에만 출력 -->
    @if ($user->role=="youtuber")
	<li class="li1">
		<div class="w1">
			<b class="t1">영상 관리 알림</b>
			<div class="t2">
				두런에 게시된 내 유튜브 영상 업데이트 및 활동에 대한 알림을 메일로 받습니다.
			</div>
		</div>
		<div class="w2">
			<a href="javescript:void(0);" class="cp1switch1 switch toggle fw4 " name="notice#4">
				<span class="t1 sw-off">OFF</span>
				<span class="t1 sw-on">ON</span>
				<i class="ic1"></i>
			</a>
...
<script>
        ...
        $(document).ready(function(){
            let my = $('.cp1notice1set1 .w2 .cp1switch1.switch.toggle.fw4');

			var array = ["{{ Auth::user()->notification_1 }}", "{{ Auth::user()->notification_2 }}"]
			@if ($user->role == "instructor")
				array[2] = "{{ Auth::user()->notification_3 }}";
			@elseif ($user->role =="youtuber")
				array[2] = "{{ Auth::user()->notification_4 }}";
			@endif
...
<script>
	$(document).ready(function(){
		$('.cp1notice1set1 .w2 .cp1switch1.switch.toggle.fw4').on('click',  function(){
            //class에 on을 포함하고 있을 경우 Y
            let isOn = (this.classList.length>5)? "Y":"N";
            let num = this.name.replace(/[^0-9]/g, '');
            $.ajax({
                headers: {
                ...
                type: 'POST',
                dataType: 'json',
                url: "{{ route('account.save_notification_settings') }}",
                data: {
                    'isOn': isOn,
                    'num': num
                },
