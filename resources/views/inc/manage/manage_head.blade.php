...
<!-- gn1 -->
<div id="gn1">
	<a href="{{ route('manage.instructor.operation_lecture') }}" class="a1" title="운영강좌로 이동"><span class="ic1"></span> <span class="t1">돌아가기</span></a>
</div>
<!-- /gn1 -->
			...
			<li class="m2">
				...
				<div class="d2">
					...
					<li class="m1">
                        ...
                        <a href="javascript:void(0);" onclick="alertDeveloping()"><span class="t1">문제 관리</span></a>
					</li>
					<li class="m2">
						{{-- <a href="{{ route('manage.lecture.exam_task_management', ['idx' => $lectureIdx]) }}"><span class="t1">시험/과제 목록</span></a> --}}
                        <a href="javascript:void(0);" onclick="alertDeveloping()"><span class="t1">시험/과제 목록</span></a>
					</li>
					</ul>
				</div>
			</li>
                    ...
                    <ul>
                        ...
                        <li class="m2">
                            <a href="{{ route('manage.lecture.qna_list', ['idx' => $lectureIdx]) }}">
                                <div class="w1">
                                    <span class="tg1">
                                        <span class="t1" id="question_list">질문 리스트</span>
                                        {{-- <i class="t2" title="질문 수">99</i> --}}
                                    </span>
                                    {{-- <b class="t3" title="미해결 된 질문 수">0</b> --}}
                                </div>
                            </a>
                        </li>
                        <li class="m3">
        ...
        $.ajax({
            ...
            success: (data) => {
                if (data.status == 'success') {
                    ...
                    var notSolutionCnt = data.notS
                    var imgSrc = '';

                    // 강좌 썸네일과 제목 적용
                    if (lectureInfo.save_thumbnail == null) {
                        imgSrc = '{{ asset('assets/images/lib/noimg1_manage.png') }}';
                    } else {
                        imgSrc = '{{ asset('storage/uploads/thumbnail/') }}' + '/' + lectureInfo.save_thumbnail;
                    }
                    $('#lecture_img').attr('src', imgSrc);
                    $('#lecture_title').append(lectureInfo.title);

                    // 미해결 된 질문 수 적용
                    if (data.notSolutionCnt > 0) {
                        $('#question_list').append('<i class="t2" title="질문 수">' + data.notSolutionCnt + '</i>');
                    }
